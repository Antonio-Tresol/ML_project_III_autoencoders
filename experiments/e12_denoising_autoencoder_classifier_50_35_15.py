def main():
    ...
    from data.data_modules import ImagesDataModule, IndexManager, Sampling
    ...
    preset_indices = IndexManager.load_indices(os.path.join(config.INDICES_DIR, config.DATASET_50_50_NAME + ".pkl"))[1]
    
    plant_dm = ImagesDataModule(
        dataset=config.DATASET_50_35_15_NAME,
        root_dir=config.ROOT_DIR,
        batch_size=config.BATCH_SIZE,
        train_folder_dataset=train_dataset,
        test_folder_dataset=test_dataset,
        train_size=config.TRAIN_SIZE_50_35_15,
        test_size=config.TEST_SIZE_50_35_15,
    ...
    for i in range(config.NUM_TRIALS):
        checkpoint_filename = (
            config.DENOISING_AUTOENCODER_50_50_DIR + config.DENOISING_AUTOENCODER_50_50_FILENAME + str(i) + ".ckpt"
        )
        ...
        model = ClassificationLightningModule(
            ...
            model_name=config.DENOISING_AUTOENCODER_CLASSIFIER_50_35_15_FILENAME.replace("_", ""),
        ...
        checkpoint_callback = ModelCheckpoint(
            monitor="val/loss",
            dirpath=config.DENOISING_AUTOENCODER_CLASSIFIER_50_35_15_DIR,
            filename=config.DENOISING_AUTOENCODER_CLASSIFIER_50_35_15_FILENAME + str(i),
            save_top_k=config.TOP_K_SAVES,
            mode="min",
        )

        id = (
            config.DENOISING_AUTOENCODER_CLASSIFIER_50_35_15_FILENAME + str(i) + "_" + wandb.util.generate_id()
        )
