def main():
    ...
    for i in range(config.NUM_TRIALS):
        checkpoint_filename = (
            config.DENOISING_AUTOENCODER_80_20_DIR + config.DENOISING_AUTOENCODER_80_20_FILENAME + str(i) + ".ckpt"
        )
        unet = Unet(in_channels=3, device=device)
        encoder = AutoencoderLightningModule.load_from_checkpoint(
            checkpoint_path=checkpoint_filename,
            model=unet,
            model_name=config.DENOISING_AUTOENCODER_80_20_FILENAME.replace("_", ""),
            loss_fn=nn.MSELoss(),
            metrics=metrics,
            lr=config.LR,
        ...
        model = ClassificationLightningModule(
            ...
            model_name=config.DENOISING_AUTOENCODER_CLASSIFIER_80_10_10_FILENAME.replace("_", ""),
        ...
        checkpoint_callback = ModelCheckpoint(
            monitor="val/loss",
            dirpath=config.DENOISING_AUTOENCODER_CLASSIFIER_80_10_10_DIR,
            filename=config.DENOISING_AUTOENCODER_CLASSIFIER_80_10_10_FILENAME + str(i),
            save_top_k=config.TOP_K_SAVES,
            mode="min",
        )

        id = (
            config.DENOISING_AUTOENCODER_CLASSIFIER_80_10_10_FILENAME + str(i) + "_" + wandb.util.generate_id()
        )
