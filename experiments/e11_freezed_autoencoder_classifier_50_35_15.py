def main():
    ...
    for i in range(config.NUM_TRIALS):
        checkpoint_filename = (
            config.AUTOENCODER_50_50_DIR + config.AUTOENCODER_50_50_FILENAME + str(i) + ".ckpt"
        )
        unet = Unet(in_channels=3, device=device)
        encoder = AutoencoderLightningModule.load_from_checkpoint(
            checkpoint_path=checkpoint_filename,
            model=unet,
            model_name=config.AUTOENCODER_50_50_FILENAME.replace("_", ""),
            loss_fn=nn.MSELoss(),
            metrics=metrics,
            lr=config.LR,
        ...
        model = ClassificationLightningModule(
            ...
            model_name=config.AUTOENCODER_CLASSIFIER_50_35_15_FILENAME.replace("_", ""),
        ...
        checkpoint_callback = ModelCheckpoint(
            monitor="val/loss",
            dirpath=config.AUTOENCODER_CLASSIFIER_50_35_15_DIR,
            filename=config.AUTOENCODER_CLASSIFIER_50_35_15_FILENAME + str(i),
            save_top_k=config.TOP_K_SAVES,
            mode="min",
        )

        id = (
            config.AUTOENCODER_CLASSIFIER_50_35_15_FILENAME + str(i) + "_" + wandb.util.generate_id()
        )
