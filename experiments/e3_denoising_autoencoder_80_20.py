def main():
    ...
    plant_dm = ImagesDataModule(
        ...
        test_size=config.TRAIN_SIZE_80_20,
    ...
    for i in range(config.NUM_TRIALS):
        ...
        checkpoint_callback = ModelCheckpoint(
            monitor="val/loss",
            dirpath=config.DENOISING_AUTOENCODER_80_20_DIR,
            filename=config.DENOISING_AUTOENCODER_80_20_FILENAME + str(i),
            save_top_k=config.TOP_K_SAVES,
            mode="min",
        )

        id = config.DENOISING_AUTOENCODER_80_20_FILENAME + str(i) + "_" + wandb.util.generate_id()
