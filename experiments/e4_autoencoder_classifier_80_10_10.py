def main():
    ...
    for i in range(config.NUM_TRIALS):
        ...
        checkpoint_callback = ModelCheckpoint(
            monitor="val/loss",
            dirpath=config.AUTOENCODER_CLASSIFIER_80_10_10_DIR,
            filename=config.AUTOENCODER_CLASSIFIER_80_10_10_FILENAME + str(i),
            save_top_k=config.TOP_K_SAVES,
            mode="min",
        )

        id = (
            config.AUTOENCODER_CLASSIFIER_80_10_10_FILENAME + str(i) + "_" + wandb.util.generate_id()
        )
